...
describe('RRT* SE2 Integration Tests', () => {
  test('RRT* problem in SE2', () => {
    const space = new oxmpl.base.SE2StateSpace(1.0, [-5, 5, -5, 5, -Math.PI, Math.PI]);
    ...
    const validityCheckerFn = (state) => {
      return state.x * state.x + state.y * state.y > 0.25;
    };
    ...
    const planner = new oxmpl.geometric.RRTStar(0.5, 0.1, 1.0, problemDef, planner_config);
