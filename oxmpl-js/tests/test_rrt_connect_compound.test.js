...
describe('RRT-Connect Compound Integration Tests', () => {
  test('RRT-Connect problem in Compound Space', () => {
    ...
    const validityCheckerFn = (state) => {
      ...
      const x = v[0];
      const y = v[1];
      if (x >= -0.5 && x <= 0.5 && y >= -2 && y <= 2) return false;
      return true;
    };
